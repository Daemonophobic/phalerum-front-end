...
const AgentTable = () => {
    ...
    return (
        <div className="w-full h-min">
            ...
            <table className="font-inter shadow-md overflow-hidden rounded-lg ml-2" width={table.getTotalSize()} >
                <thead className="bg-gray-100">
                    {table.getHeaderGroups().map((headerGroup) => <tr key={headerGroup.id}>
                        {headerGroup.headers.map(
                            header => <th style={{width: header.getSize()}} className="font-semibold text-left relative h-full group-hover:opacity-100 group p-2" key={header.id}>
                ...
                <tbody>
                    {
                        table.getRowModel().rows.map(row => <tr key={row.id}>
                            {row.getVisibleCells().map(cell => <td className="p-2" style={{width: cell.column.getSize()}} key={cell.id}>
                                {
                                    flexRender(
